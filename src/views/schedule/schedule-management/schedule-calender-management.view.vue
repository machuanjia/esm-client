...
import scheduleCalenderCollection from '@/views/schedule/schedule-management/schedule-calender-categoty-collection.vue';
import {
...
export default class extends mixins(ViewMixin) {
  ...
  async editAction(row: any) {
    const { data } = await getCalenderCategory(row.id, {});
    if (data) {
  ...
  saveAction(payload: any) {
    if (this.entity) {
      ...
      this.addCalenderCategory(payload);
    }
  ...
  removeAction(row: any) {
    this.removeCollectionAction(async(successFn: any) => {
      const { data } = await deleteCalenderCategory(row.id);
  ...
  changeStatus(row: any) {
    this.updateCalenderCategory(row.id, {
      name: row.name,
      status: row.status,
  ...
  private async addCalenderCategory(payload: any) {
    const { data } = await addCalenderCategory(payload);
    if (data) {
      this.pushCollection(this.categotyes, data);
    }
  }

  private async updateCalenderCategory(id: number, payload: any) {
    const { data } = await updateCalenderCategory(id, payload);
    if (data) {
