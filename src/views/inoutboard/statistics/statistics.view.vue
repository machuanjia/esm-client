  ...
  >
    <template v-slot:hleft>
      <span class="app-content-full-title">
        ...
        <span class="ml5">{{ $t('route.statistics') }}</span>
      </span>
    </template>
          ...
          >
            <el-table-column label="成员">
              <template slot-scope="{row}">
                <div :class="{'text-right': row.member === 'Total'}">
                  <span v-if="row.member === 'Total'">{{ row.member }}</span>
                  <span v-if="row.member && row.member !== 'Total'">{{ row.member.name }}</span>
                </div>
              </template>
            </el-table-column>
            <el-table-column
...
import { getInoutBoardStatuses, getStatistics } from '@/api/inoutboard';
